...
void onInit(CBlob@ this)
{
	...
	Vehicle_addWheel(this, v, "WoodenWheels.png", 8, 8, 1, Vec2f(-5.0f, 8.5f));
	Vehicle_addWheel(this, v, "WoodenWheels.png", 8, 8, 0, Vec2f(4.0f, 8.0f));
	...
	if (this.hasAttached() || time < 30 || time_til_fire > 0) //driver, seat or gunner, or just created
	{
		...
		if (getNet().isClient() && delay != 0) //only matters visually on client
		{
			...
			if (arm !is null)
			{
				...
				Vec2f armOffset = Vec2f(-6.0f, -5.0f);
				arm.SetOffset(armOffset);

				arm.ResetTransform();
				arm.SetRelativeZ(-10.5f);
				arm.RotateBy(armAngle * -sign, Vec2f(0.0f, 6.5f));
				...
				Vec2f offset = Vec2f(2, -5);
				offset.RotateBy(-armAngle, Vec2f(0.0f, 6.5f));
				offset += armOffset + Vec2f(14, 0);
...
void PopWheels(CBlob@ this, bool addparticles = true)
{
	...
	if (chocks !is null)
	{
		...
		anim.AddFrame(0);
		chocks.SetRelativeZ(14.0f);
		chocks.SetOffset(Vec2f(0.0f, 6.8f));
	}
}
