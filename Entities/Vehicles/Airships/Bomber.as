...
//	this.getShape().getConsts().transports = true;
	...
	CSpriteLayer@ balloon = sprite.addSpriteLayer("balloon", "Balloon.png", 24, 32);
	if (balloon !is null)
	{
		...
		balloon.SetOffset(Vec2f(0.0f, -13.0f));
	}

	CSpriteLayer@ background = sprite.addSpriteLayer("background", "Balloon.png", 16, 8);
	if (background !is null)
	{
		...
		background.SetOffset(Vec2f(0.0f, -2.0f));
	}

	CSpriteLayer@ burner = sprite.addSpriteLayer("burner", "Balloon.png", 4, 8);
	if (burner !is null)
	{
		{
		...
		burner.SetOffset(Vec2f(0.0f, -13.0f));
	}
}
...
void onTick(CSprite@ this)
{
	...
	if (burner !is null)
	{
		burner.SetOffset(Vec2f(0.0f, -7.0f));
