...
void LoadSprites(CSprite@ this)
{
	...
	CSpriteLayer@ frontarm = this.addSpriteLayer("frontarm", texname , 16, 8, this.getBlob().getTeamNum(), this.getBlob().getSkinNum());
	...
	CSpriteLayer@ backarm = this.addSpriteLayer("backarm", texname , 16, 8, this.getBlob().getTeamNum(), this.getBlob().getSkinNum());
	...
	CSpriteLayer@ arrow = this.addSpriteLayer("held arrow", "Arrow.png" , 8, 4, this.getBlob().getTeamNum(), this.getBlob().getSkinNum());
	...
	CSpriteLayer@ quiver = this.addSpriteLayer("quiver", texname , 8, 8, this.getBlob().getTeamNum(), this.getBlob().getSkinNum());
	...
	CSpriteLayer@ hook = this.addSpriteLayer("hook", texname , 8, 4, this.getBlob().getTeamNum(), this.getBlob().getSkinNum());
	...
	CSpriteLayer@ rope = this.addSpriteLayer("rope", texname , 16, 4, this.getBlob().getTeamNum(), this.getBlob().getSkinNum());
...
void doQuiverUpdate(CSprite@ this, bool has_arrows, bool quiver)
{
	...
	if (quiverLayer !is null)
	{
		if (quiver)
		{
			...
			{
				easy = true;
				off.Set((this.getFrameWidth() / 4), -this.getFrameHeight() / 4);
				off += this.getOffset();
				off += Vec2f(-po.x, po.y);


				f32 y = (down ? 0.5f : 2.0f);
				f32 x = (down ? 6.0f : 5.5f);
